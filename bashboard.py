...
# Load the dataset
data_path = 'numberofinternetusers new.csv'
...
# 3. Time Series with Linear Regression
selected_country = 'United States'
time_series_data = data[data['Entity'] == selected_country]

# Fit linear regression
X = sm.add_constant(time_series_data['Year'])  # adding a constant
model = sm.OLS(time_series_data['Number of Internet users'], X).fit()
...
line_fig.add_scatter(x=time_series_data['Year'], y=predictions, mode='lines', name='Regression Line')
